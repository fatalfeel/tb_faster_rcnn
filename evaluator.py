...
class Evaluator(object):
    def __init__(self, dataset, batch_size, path_to_data_dir, path_to_results_dir, device, kwargs):
        super().__init__()
        self._dataset               = dataset
        self._dataloader            = DataLoader(dataset, batch_size=batch_size, shuffle=False, **kwargs)
    ...
    def evaluate(self, model: Model) -> Tuple[float, str]:
        ...
        with torch.no_grad():
            for _, (image_id_batch, image_batch, scale_batch, _, _) in enumerate(tqdm(self._dataloader)):
                ...
                detection_classes, \
                detection_probs, \
                detection_batch_indices = model.eval().forward(image_batch)
        ...
        mean_ap, detail = self._dataset.evaluate(self._path_to_results_dir,
                                                 all_image_ids,
