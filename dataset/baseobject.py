...
class DatasetBase(torch.utils.data.dataset.Dataset):
    ...
    def preprocess(image: PIL.Image.Image, image_min_side: float, image_max_side: float) -> Tuple[Tensor, float]:
        ...
        transform = transforms.Compose([transforms.Resize((round(image.height * scale), round(image.width * scale))),  # interpolation `BILINEAR` is applied by default
                                        transforms.ToTensor(),
    ...
    def padding_collate_fn(batch: List[Tuple[str, Tensor, Tensor, Tensor, Tensor]]) -> Tuple[List[str], Tensor, Tensor, Tensor, Tensor]:
        ...
        for image in image_batch:
            padded_image = tnf.pad(input=image, pad=(0, max_image_width - image.shape[2], 0, max_image_height - image.shape[1]))  # pad has format (left, right, top, bottom)
    ...
    class NearestRatioRandomSampler(torch.utils.data.sampler.Sampler):
        ...
        def __iter__(self) -> Iterator[int]:
            ...
            tall_indices    = torch.nonzero(image_ratios < 1).view(-1)
            fat_indices     = torch.nonzero(image_ratios >= 1).view(-1)
